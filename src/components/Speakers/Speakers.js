...
const SpeakerCard = ({ data }) => {
  return (
    <div className="speaker__card">
      ...
      <h3 className="speaker__name">{data.name}</h3>
      <div className="speaker__line"></div>
      <p className="speaker__description">{data.description}</p>
      <a href="#2" className="speaker__link">
        Подробнее
      </a>
    </div>
  );
...
const Speakers = () => {
  ...
  const onWindowScroll = () => {
    dispatch(
      uiActions.setSpeakersOffset(
        speakersRef.current.getBoundingClientRect().top + window.pageYOffset
      )
    );
  };
      ...
      <div className="wrapper">
        ...
        {data.length ? (
          <div className="speakers__grid">
            {data.map((el, idx) => {
              return <SpeakerCard key={idx} data={el} />;
            })}
          </div>
        ) : (
          <p className="speakers__notification">Загрузка...</p>
        )}
      </div>
    </div>
  );
